...
export class PropertyService {
  ...
  async listMyProperties(
    ...
    const itemsWithCover = items.map((p) => {
      const mediaLinksRaw = Array.isArray(p.mediaLinks) ? p.mediaLinks : [];
      const coverLink = mediaLinksRaw[0];
      const cover = coverLink?.file
        ? {
            // Build URL from S3 bucket/key or use local path
            url: `/${coverLink.file.bucket}/${coverLink.file.key}`,
