...
export class ReviewService {
  ...
  private async assertBookingUsable(tx: Tx, bookingId: string, userId: string) {
    const b = await tx.propertyBooking.findUnique({
      where: { id: bookingId },
      select: {
        id: true,
        propertyId: true,
  ...
  async create(userId: string, dto: CreateReviewDto, idemKey?: string) {
    ...
    const run = async (tx: Tx) => {
      ...
      await this.outbox.createEvent('review.created', reviewId, {
        type: 'review.created',
        reviewId,
        bookingId: booking.id,
        propertyId: booking.propertyId,
  ...
  async update(
    ...
    const run = async (tx: Tx) => {
      ...
      await this.outbox.createEvent('review.updated', updated.id, {
        type: 'review.updated',
        reviewId: updated.id,
  ...
  async remove(userId: string, id: string, idemKey?: string) {
    ...
    const run = async (tx: Tx) => {
      ...
      await this.outbox.createEvent('review.deleted', prev.id, {
        type: 'review.deleted',
        reviewId: prev.id,
